...
export const mockProvider: HostingProvider = {
  ...
  async updateDomainNameservers(_domain, nameservers) { return { ok: true, data: { nameservers } }; },
  async getDomainEppCode() { return { ok: true, data: { eppcode: 'MOCK-EPP-CODE' } }; },
  async getDomainLock() { return { ok: true, data: { locked: true } }; },
  async updateDomainLock(_domain, locked) { return { ok: true, data: { locked } }; },
