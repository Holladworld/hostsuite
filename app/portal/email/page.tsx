...
type EmailService = { id: string; service_name: string; status: string; provider: string | null; provider_status: string | null; provider_resource_id: string | null; control_panel_url: string | null; last_error: string | null; configuration: Record<string, unknown> | null; created_at: string; };
type ProviderState = { configured: boolean; capabilities: string[] };

function value(input: unknown, fallback: string) { return typeof input === 'string' && input.trim() ? input.trim() : fallback; }
function mailboxes(configuration: Record<string, unknown> | null) { const addresses = configuration?.emailAddresses; return Array.isArray(addresses) ? addresses.filter((item): item is string => typeof item === 'string' && item.trim()).map((item) => item.trim()) : []; }
...
export default function BusinessEmailPage() {
  ...
  async function load() {
    ...
    if (error) toast.error('Unable to load your business email services.'); else setServices((data ?? []) as EmailService[]);
    if (capabilityResponse?.ok) { const result = await capabilityResponse.json() as { capabilities?: string[] }; setProvider({ configured: true, capabilities: result.capabilities ?? [] }); } else setProvider({ configured: false, capabilities: [] });
    setBusy(false);
  }
  ...
  const emailCapabilities = provider.capabilities.filter((cap) => cap.startsWith('email.'));
...
function Info({ label, value: content }: { label: string; value: string }) { return <div className="rounded-xl border p-3"><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 break-all text-sm font-medium">{content}</p></div>; }
