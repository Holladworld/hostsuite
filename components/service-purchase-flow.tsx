...
export function ServicePurchaseFlow({ service, managementMode = 'self' }: Props) {
  ...
  return <main className="min-h-screen bg-background px-4 py-10 sm:px-6 lg:px-8"><div className="mx-auto max-w-4xl">
    ...
    {step === 2 && <section className="mt-6 space-y-5"><Card><CardHeader><CardTitle>{service === 'domain' ? 'Find your domain' : 'Connect your domain'}</CardTitle><CardDescription>{service === 'domain' ? 'We never show an unverified domain as available.' : 'Use a domain you already own, or tell us the domain you want to buy.'}</CardDescription></CardHeader><CardContent>{service === 'domain' ? <><div className="flex gap-2"><Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="mybusiness.com"/><Button type="button" variant="outline" onClick={() => { setSearched(true); setDomain(query); }}><Search className="mr-2 h-4 w-4" /> Search</Button></div>{searched && <div className="mt-4 rounded-xl border border-amber-500/30 bg-amber-500/5 p-4 text-sm"><p className="font-medium">Provider availability check is not configured.</p><p className="mt-1 text-muted-foreground">HostSuite will not pretend this domain is available. It will verify the requested domain against the real reseller provider when the provider adapter is configured.</p></div>}{suggestions.length > 0 && <div className="mt-5"><p className="text-sm font-medium">Other suggestions</p><div className="mt-2 flex flex-wrap gap-2">{suggestions.map((suggestion) => <button key={suggestion} type="button" onClick={() => { setQuery(suggestion); setDomain(suggestion); }} className="rounded-full border px-3 py-1.5 text-sm hover:border-primary">{suggestion}</button>)}</div><p className="mt-2 text-xs text-muted-foreground">Suggestions are generated names, not confirmed availability.</p></div>}</> : <><div className="grid gap-3 sm:grid-cols-2"><button type="button" onClick={() => setDomainMode('existing')} className={`rounded-xl border p-4 text-left ${domainMode === 'existing' ? 'border-primary ring-2 ring-primary/20' : ''}`}><p className="font-semibold">I already have a domain</p><p className="mt-1 text-xs text-muted-foreground">Use an existing domain with this service.</p></button><button type="button" onClick={() => setDomainMode('new')} className={`rounded-xl border p-4 text-left ${domainMode === 'new' ? 'border-primary ring-2 ring-primary/20' : ''}`}><p className="font-semibold">I need a domain</p><p className="mt-1 text-xs text-muted-foreground">I want HostSuite to register one.</p></button></div><div className="mt-4"><label className="text-sm font-medium">Domain name</label><Input className="mt-2" value={domain} onChange={(e) => setDomain(e.target.value)} placeholder="mybusiness.com" /></div></>}</CardContent></Card>
