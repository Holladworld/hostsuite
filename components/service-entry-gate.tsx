...
import { ArrowLeft, ArrowRight, CheckCircle2, Globe, Loader2, Mail, Plus, Server, WandSparkles } from 'lucide-react';
...
export function ServiceEntryGate({ service }: { service: ServiceType }) {
    ...
    void (async () => {
      ...
      if (error) {
        ...
        if (!(error.message?.toLowerCase().includes('relation') || error.code === '42P01')) {
          toast.error('We could not check your existing HostSuite services.');
        }
      } else {
  ...
  const continueExisting = () => {
    ...
    toast.info('We have your details. External service management setup will be completed in the next step.');
    setChoice('existing');
  };
