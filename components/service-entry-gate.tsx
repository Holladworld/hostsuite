...
export function ServiceEntryGate({ service }: { service: ServiceType }) {
  ...
  useEffect(() => {
    if (!user) return;
    void (async () => {
      const [instanceResult, assetResponse] = await Promise.all([
      ...
      if (instanceResult.error) {
        setInstances([]);
        if (!(instanceResult.error.message?.toLowerCase().includes('relation') || instanceResult.error.code === '42P01')) toast.error('We could not check your services right now.');
      } else setInstances((instanceResult.data as Instance[]) ?? []);
      if (assetResponse?.ok) {
        const result = await assetResponse.json() as { assets?: ManagedAsset[] };
        setAssets(result.assets?.filter((asset) => asset.service_type === service) ?? []);
      }
      setChecking(false);
    })();
  }, [user, service]);
  ...
  return <main className="min-h-screen bg-background px-4 py-8 sm:px-6 lg:px-8"><div className="mx-auto max-w-4xl">
    ...
    {origin === 'new' && management === null ? null : null}
  </div></main>;
}
