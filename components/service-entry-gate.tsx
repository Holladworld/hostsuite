...
const labels: Record<ServiceType, { label: string; icon: typeof Server }> = {
  hosting: { label: 'Hosting', icon: Server }, domain: { label: 'Domain', icon: Globe }, email: { label: 'Business Email', icon: Mail }, website: { label: 'Website', icon: WandSparkles },
};
  ...
  async function saveExisting() {
    ...
    try {
      const response = await fetch('/api/managed-assets', {
        ...
        body: JSON.stringify({ serviceType: service, name: existingIdentifier.trim(), identifier: existingIdentifier.trim(), providerName: existingProvider.trim() || null, managementMode: management, details: { source: 'customer_existing_service' } }),
      });
    ...
    } catch (error) { toast.error(error instanceof Error ? error.message : 'Unable to save this service.'); }
